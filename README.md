...
2. **Route .ru domains and geoip:ru through WARP** — установщик создаёт WARP outbound в локальной 3x-ui и направляет через него только домены в зоне `.ru` (включая поддомены) и IP-адреса из `geoip:ru`. Остальной трафик остаётся на обычном маршруте.
