...
Этот режим зависит от доступности Cloudflare WARP и geofile `geoip:ru`; после установки итоговый аудит покажет статус **WARP RU routing**. На VPS с примерно 1 ГБ RAM скрипт после успешной настройки WARP автоматически создаёт и включает `/swapfile` размером 1 ГБ, если активного swap ещё нет и на диске достаточно места. Существующий swap и существующий `/swapfile` не изменяются.
